...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/titanic_with_kedro")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=titanic_with_kedro"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Titanic with Kedro"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "titanic_with_kedro"
